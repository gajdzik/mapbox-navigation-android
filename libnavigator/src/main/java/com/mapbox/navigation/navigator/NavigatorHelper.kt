...
import com.mapbox.navigation.base.trip.model.CurrentLegAnnotation
 ...
 */
internal object NavigatorHelper {
    ...
    fun decodeStepPoints(
        ...
        legs?.let { legs ->
            ...
            steps?.let { steps ->
                ...
                if (invalidStepIndex) return emptyList()
                val step = steps[stepIndex]
                step?.let { step ->
                    val stepGeometry = step.geometry()
                    stepGeometry?.let {
                        return PolylineUtils.decode(stepGeometry, /* todo add core dependency PRECISION_6*/6)
                    }
                }
            }
        }
